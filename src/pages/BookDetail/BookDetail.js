...
const BookDetail = () => {
    ...
    console.log(queryClient.getQueryData("principal"));
        ...
        const getBook = useQuery(["getBook"], async () => {
            ...
            const response = await axios(`http://localhost:8080/book/${bookId}`, option);
            return response;
        });
    ...
    return (
        <div css={mainContainer}>
            ...
            <header>
                ...
                <p>분류: {getBook.data.data.categoryName}
                    / 저자명: {getBook.data.data.authorName} 
                    / 출판사: {getBook.data.data.publisherName} 
                    / 추천: {getLikeCount.isLoading ? "searching..." : getLikeCount.data.data}</p>
            </header>
            <main>
                <div>

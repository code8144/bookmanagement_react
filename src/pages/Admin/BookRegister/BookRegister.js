...
const BookRegister = () => {
    ...
    const registeBookList = useMutation(async(bookId) =>{
        const option ={
            headers:{
        "Content-Type":"application/json", //415 문제 / 404 주소 /401 로그인 되지않음/ 400 값오류
        Authorization: localStorage.getItem("accessToken")
            }
        }
    ...
    const pagination = () =>{
        ...
        const startIndex = nowPage % 5 === 0 ? nowPage- 4: nowPage - (nowPage % 5) +1; // 마직막 인덱스까지 필요함 , 페이지 공식 
        const endIndex = startIndex + 4 <= lastPage ? startIndex + 4 : lastPage; // 마지막 페이지

        const pageNumbers = [];

        for(let i = startIndex; i<= endIndex; i++){
            pageNumbers.push(i); // 배열
        }
        return (
            ...
            <button  disabled = {nowPage ===1} onClick={()=> {
                ...
                {pageNumbers.map(page => (<button key={page} onClick={() =>{
                     ...
                     setRefresh(true);
                }} disabled = {page === nowPage}>{page}</button>))}   {/* 현재페이지는 선택이 안되도록 만드는 것 */}
